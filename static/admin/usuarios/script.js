...
function setupSidebar() { const b = document.getElementById("sidebarToggle"), s = document.getElementById("sidebar"), i = document.getElementById("toggleIcon"); if(b) b.addEventListener("click", () => { s.classList.toggle("collapsed"); if(i) { i.setAttribute("data-lucide", s.classList.contains("collapsed") ? "panel-left-open" : "menu"); lucide.createIcons(); } }); }
function setupProfile() { const u = JSON.parse(localStorage.getItem("aura_user") || "{}"); const n = u.nome || "Admin Principal"; const email = u.email || "admin@admin.com"; const c = u.cargo || "Administrador"; const firstName = n.split(" ")[0]; const i = n.split(" ").slice(0, 2).map(x => x[0]).join("").toUpperCase() || "AD"; ["profileName", "topbarName"].forEach(id => { const el = document.getElementById(id); if(el) el.textContent = n; }); ["profileAvatar", "topbarAvatar", "popoverAvatar"].forEach(id => { const el = document.getElementById(id); if(el) el.textContent = i; }); document.querySelectorAll('.profile-role').forEach(el => el.textContent = `${c} · IBK`); const pEmail = document.getElementById("popoverEmail"); if(pEmail) pEmail.textContent = email; const pGreet = document.getElementById("popoverGreeting"); if(pGreet) pGreet.textContent = `Olá, ${firstName}!`; const pRole = document.getElementById("popoverRole"); if(pRole) pRole.textContent = c; const pCard = document.getElementById("profileCard"); const pPop = document.getElementById("userProfilePopover"); const pClose = document.getElementById("popoverClose"); if (pCard && pPop) { pCard.addEventListener("click", (e) => { e.stopPropagation(); pPop.classList.toggle("show"); }); if (pClose) pClose.addEventListener("click", (e) => { e.stopPropagation(); pPop.classList.remove("show"); }); document.addEventListener("click", (e) => { if (!pPop.contains(e.target) && !pCard.contains(e.target)) pPop.classList.remove("show"); }); } document.querySelectorAll('.logout').forEach(btn => btn.addEventListener('click', () => { localStorage.removeItem('aura_token'); localStorage.removeItem('aura_user'); window.location.replace('/login.html'); })); }
