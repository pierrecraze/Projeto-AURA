...
function setupProfile() { const u = JSON.parse(localStorage.getItem("aura_user") || "{}"), n = u.nome || "Admin Principal", e = u.email || "admin@admin.com", c = u.cargo || "Administrador", i = n.split(" ").slice(0, 2).map(x => x[0]).join("").toUpperCase() || "AD"; ["profileName", "topbarName", "popoverName"].forEach(id => { const el = document.getElementById(id); if(el) el.textContent = n; }); ["profileAvatar", "topbarAvatar", "popoverAvatar"].forEach(id => { const el = document.getElementById(id); if(el) el.textContent = i; }); const popE = document.getElementById("popoverEmail"); if(popE) popE.textContent = e; document.querySelectorAll('.profile-role').forEach(el => el.textContent = `${c} · IBK`); const pCard = document.getElementById("profileCard"); const pBtn = document.getElementById("profileMoreBtn"); const pPop = document.getElementById("profilePopover"); const pDrop = document.getElementById("profileDropdown"); if (pCard && pPop && pDrop) { pCard.addEventListener("click", (evt) => { if (evt.target === pBtn || pBtn.contains(evt.target)) { evt.stopPropagation(); pDrop.classList.toggle("show"); pPop.classList.remove("show"); } else { evt.stopPropagation(); pPop.classList.toggle("show"); pDrop.classList.remove("show"); } }); document.addEventListener("click", () => { pPop.classList.remove("show"); pDrop.classList.remove("show"); }); } document.querySelectorAll('.logout').forEach(btn => btn.addEventListener('click', () => { localStorage.removeItem('aura_token'); localStorage.removeItem('aura_user'); window.location.replace('/login.html'); })); }
