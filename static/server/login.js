...
form.addEventListener("submit", async (e) => {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    setInvalid("grpEmail", true);
    valid = false;
  } else {
    setInvalid("grpEmail", false);
  }

  if (!senha) {
    setInvalid("grpSenha", true);
    valid = false;
  } else {
    setInvalid("grpSenha", false);
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      setTimeout(() => {
        if (data.role === "medico") {
          window.location.href = "/dashboardMedico.html"; // Manda o médico pros pacientes
        } else if (data.role === "admin") {
          window.location.href = "/admin/dashboard/index.html"; // Manda o admin pro painel
        } else {
